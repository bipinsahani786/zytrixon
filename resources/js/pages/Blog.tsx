            ...
            <main style={{ minHeight: '100vh', background: 'var(--zy-black)', overflowX: 'hidden' }}>
                ...
                <section style={{ padding: '60px 24px', maxWidth: 1200, margin: '0 auto' }}>
                    <div style={{
                        ...
                        <div style={{ height: 400, background: 'linear-gradient(135deg, #1a1a1a 0%, #000000 100%)', position: 'relative' }}>
                            {/* Placeholder Image / Graphic */}
                            <div style={{
                                position: 'absolute', inset: 0,
                                backgroundImage: 'radial-gradient(circle at 20% 30%, rgba(0, 112, 243, 0.2) 0%, transparent 50%)'
                            }} />
                            <div style={{ position: 'absolute', bottom: 24, left: 24, background: 'rgba(0,0,0,0.5)', padding: '8px 16px', borderRadius: 20, backdropFilter: 'blur(10px)', color: 'var(--zy-blue)', fontSize: 13, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em' }}>
                                Featured Article
                            </div>
                        </div>
                        <div style={{ padding: '40px 40px 40px 0', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                            <div style={{ color: 'var(--zy-gray-text)', fontSize: 14, marginBottom: 16 }}>May 24, 2026 • 8 min read</div>
                            <h2 style={{ fontSize: 'clamp(28px, 4vw, 40px)', fontFamily: 'var(--font-heading)', fontWeight: 800, color: 'var(--zy-white)', marginBottom: 24, lineHeight: 1.2 }}>
                                The Future of Enterprise Architecture: Serverless Meets Edge Computing
                            </h2>
                            <p style={{ fontSize: 16, color: 'var(--zy-gray-light)', marginBottom: 32, lineHeight: 1.6 }}>
                ...
                <section style={{ padding: '20px 24px 60px', maxWidth: 1200, margin: '0 auto' }}>
                    <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', justifyContent: 'center' }}>
                        {['All Articles', 'Engineering', 'AI & Automation', 'Design UI/UX', 'Business Strategy', 'Case Studies'].map((cat, i) => (
                ...
                <section style={{ padding: '0 24px 100px', maxWidth: 1200, margin: '0 auto' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 32 }}>
                        {[
                            ...
                            { title: 'Designing for the Dark Mode: Best Practices', category: 'Design UI/UX', date: 'May 05, 2026' },
                            { title: 'Why Next.js 15 is a Game Changer for SEO', category: 'Engineering', date: 'Apr 28, 2026' },
                            { title: 'Automating Customer Support with Custom LLMs', category: 'AI & Automation', date: 'Apr 21, 2026' },
                            { title: 'Migrating from Monolith to Microservices', category: 'Engineering', date: 'Apr 15, 2026' }
                        ].map((post, index) => (
                            <div key={index} style={{
                                background: 'var(--zy-gray-card)',
                                borderRadius: 16,
                ...
                <section style={{ padding: '80px 24px', background: 'var(--zy-gray-card)', textAlign: 'center', borderTop: '1px solid var(--zy-gray-border)' }}>
                    <div style={{ maxWidth: 600, margin: '0 auto' }}>
                        <h2 style={{ fontSize: 32, fontFamily: 'var(--font-heading)', fontWeight: 800, color: 'var(--zy-white)', marginBottom: 16 }}>
                            Stay Ahead of the Curve
                        </h2>
                        <p style={{ color: 'var(--zy-gray-text)', marginBottom: 32, fontSize: 16 }}>
                            Get the latest engineering insights and tech news delivered straight to your inbox once a month. No spam, ever.
                        </p>
                        <form style={{ display: 'flex', gap: 12, maxWidth: 480, margin: '0 auto' }}>
                            <input 
                                type="email" 
                                placeholder="Enter your email address" 
                                style={{ flex: 1, padding: '16px 24px', borderRadius: 30, border: '1px solid var(--zy-gray-border)', background: '#111', color: 'var(--zy-white)', outline: 'none' }}
                                required
                            />
                            <button type="submit" className="magnetic-btn" style={{ padding: '16px 32px', border: 'none' }}>
                                Subscribe
                            </button>
                        </form>
                    </div>
