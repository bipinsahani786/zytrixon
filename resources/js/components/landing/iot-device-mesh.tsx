...
export default function IoTDeviceMesh({ mouseX, mouseY }: IoTDeviceMeshProps) {
    ...
    const circuitLines = useMemo(() => {
        const lines: { points: [number, number, number][]; opacity: number; animated?: boolean }[] = [];
        
        // Helper to add angled paths
        const addPath = (startX: number, startY: number, midX: number, midY: number, endX: number, endY: number, opacity: number, animated = false) => {
            lines.push({ points: [[startX, startY, 0.03], [midX, midY, 0.03], [endX, endY, 0.03]], opacity, animated });
        };
        ...
        // Edge traces
        lines.push({ points: [[-1.8, 1.2, 0.03], [-1.8, -1.2, 0.03], [1.8, -1.2, 0.03], [1.8, 1.2, 0.03]], opacity: 0.2 });
    ...
    const chips = useMemo(() => {
        return [
            // Main CPU
            { pos: [0, 0, 0.05], size: [0.8, 0.8, 0.06], color: chipColor, type: 'cpu' },
            // Memory ICs
            { pos: [-1.2, 0.7, 0.05], size: [0.4, 0.6, 0.06], color: chipColor, type: 'ram' },
            { pos: [-1.2, -0.7, 0.05], size: [0.4, 0.6, 0.06], color: chipColor, type: 'ram' },
            // Comms / Radio
            { pos: [1.2, 0.7, 0.05], size: [0.5, 0.5, 0.06], color: chipColor, type: 'radio' },
            // Power management
            { pos: [1.2, -0.7, 0.05], size: [0.3, 0.3, 0.08], color: chipColor, type: 'pmu' },
        ];
    }, [chipColor]);
    ...
    useFrame((state, delta) => {
        ...
        if (ballRef.current) {
            ballRef.current.position.z = 0.12 + Math.sin(state.clock.elapsedTime * 3) * 0.03;
        }
        if (materialRef.current) {
            materialRef.current.emissiveIntensity = isLight ? 0.8 : (2 + Math.sin(state.clock.elapsedTime * 6) * 1);
        }
        ...
        if (ringsRef.current) {
            ringsRef.current.children.forEach((ring, i) => {
                ...
                (ring as any).material.opacity = 0.3 + Math.sin(state.clock.elapsedTime * 2 + i) * 0.2;
            });
        }
    ...
    return (
        <group ref={groupRef}>
            ...
            <mesh position={[0, 0, -0.05]}>
                ...
                <meshStandardMaterial color={isLight ? '#e0e0e0' : '#050505'} roughness={0.9} metalness={0.1} />
            </mesh>
            <mesh>
                <boxGeometry args={[4.0, 2.8, 0.04]} />
                <meshStandardMaterial color={pcbColor} roughness={0.7} metalness={0.6} />
            </mesh>
            
            {/* PCB Edge Details */}
            <lineSegments>
                <edgesGeometry args={[new THREE.BoxGeometry(4.0, 2.8, 0.04)]} />
                <lineBasicMaterial color={highlightColor} transparent opacity={0.3} />
            </lineSegments>
            ...
            {chips.map((chip, i) => (
                <group key={`chip-${i}`} position={chip.pos as [number, number, number]}>
                    <mesh>
                        <boxGeometry args={chip.size as [number, number, number]} />
                        <meshStandardMaterial color={chip.color} roughness={0.4} metalness={0.8} />
                    </mesh>
                    <lineSegments>
                        <edgesGeometry args={[new THREE.BoxGeometry(...chip.size as [number, number, number])]} />
                        <lineBasicMaterial color={highlightColor} transparent opacity={0.4} />
                    </lineSegments>
                    {/* Chip Pins */}
                    {chip.type === 'cpu' && (
                        <group position={[0, 0, -0.02]}>
                            {Array.from({ length: 16 }).map((_, j) => (
                                <mesh key={`pin-t-${j}`} position={[-0.35 + j * 0.046, 0.42, 0]}>
                                    <boxGeometry args={[0.02, 0.06, 0.02]} />
                                    <meshStandardMaterial color="#c0c0c0" metalness={1} />
                                </mesh>
                            ))}
                            {Array.from({ length: 16 }).map((_, j) => (
                                <mesh key={`pin-b-${j}`} position={[-0.35 + j * 0.046, -0.42, 0]}>
                                    <boxGeometry args={[0.02, 0.06, 0.02]} />
                                    <meshStandardMaterial color="#c0c0c0" metalness={1} />
                                </mesh>
                            ))}
            ...
            {components.map((comp, i) => (
                <group key={`smd-${i}`} position={comp.pos as [number, number, number]} rotation={comp.rot as [number, number, number]}>
                    <mesh>
                        <boxGeometry args={comp.isCapacitor ? [0.06, 0.04, 0.04] : [0.08, 0.03, 0.02]} />
                        <meshStandardMaterial color={comp.isCapacitor ? (isLight ? '#a0a0a0' : '#444') : (isLight ? '#111' : '#222')} roughness={0.5} />
                    </mesh>
                    {/* Solder pads */}
            ...
            <group position={[0, 0, 0.08]}>
                ...
                <group ref={ringsRef}>
                    ...
                    <Ring args={[0.28, 0.29, 48]} position={[0, 0, -0.04]}>
                        <meshBasicMaterial color={highlightColor} transparent opacity={0.15} side={THREE.DoubleSide} />
                    </Ring>
                </group>
            </group>
            ...
            <group position={[1.4, 0.7, 0.06]}>
                <mesh rotation={[Math.PI / 2, 0, 0]}>
                    ...
                    <meshStandardMaterial color={isLight ? '#555' : '#888'} metalness={0.9} roughness={0.1} />
                </mesh>
