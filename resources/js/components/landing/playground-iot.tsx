...
export default function PlaygroundIoT() {
    ...
    const [logs, setLogs] = useState<string[]>([
        '[System] IoT Dashboard initialized...',
    ]);
        ...
        const interval = setInterval(() => {
            setSensors((prev) =>
                prev.map((s) => {
                    ...
                    const delta =
                        (Math.random() - 0.5) * (s.max - s.min) * 0.05;
                    const newVal = Math.max(
                        s.min,
                        Math.min(s.max, s.value + delta),
                    );
    ...
    useEffect(() => {
        ...
        activeSensors.forEach((_, i) => {
            const angle =
                (i / activeSensors.length) * Math.PI * 2 - Math.PI / 2;
    ...
    const toggleSensor = (id: string) => {
        setSensors((prev) =>
            prev.map((s) => {
                ...
                setLogs((l) => [
                    `[${new Date().toLocaleTimeString()}] ${s.label} ${next.active ? 'ACTIVATED' : 'DEACTIVATED'}`,
                    ...l.slice(0, 6),
                ]);
