...
export default function PricingMatrix({ service }: any) {
    ...
    return (
        <section className="zy-section" style={{ background: isLight ? '#f9fafb' : 'var(--zy-black)', position: 'relative', overflow: 'hidden' }}>
            {/* Background Glow */}
            <div style={{ position: 'absolute', top: 0, left: '50%', transform: 'translateX(-50%)', width: 800, height: 400, background: '#6366f1', filter: 'blur(200px)', opacity: isLight ? 0.05 : 0.1, zIndex: 0 }} />

            <div style={{ maxWidth: 1200, margin: '0 auto', textAlign: 'center', position: 'relative', zIndex: 1 }}>
                <span className="zy-section-label" style={{ color: '#6366f1', opacity: 1 }}>Pricing</span>
                <h2 className="zy-section-title" style={{ color: isLight ? '#000' : 'var(--zy-white)', marginBottom: 16 }}>
                    Transparent Engagement Models
                </h2>
                <p style={{ color: isLight ? '#555' : 'var(--zy-gray-text)', fontSize: 18, maxWidth: 700, margin: '0 auto 64px', lineHeight: 1.6 }}>
                    Choose the engagement model that best fits your project's scope, timeline, and flexibility requirements.
                </p>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 32, alignItems: 'center' }}>
                    {[
                        ...
                        <div key={i} className="pricing-card-wrapper" style={{ 
                            position: 'relative',
                            ...
                            {plan.highlighted && (
                                <div style={{ 
                                    ...
                                    padding: '6px 20px', borderRadius: 50, fontSize: 12, fontWeight: 800, 
                                    letterSpacing: '0.1em', zIndex: 10, boxShadow: '0 5px 15px rgba(99, 102, 241, 0.4)'
                                }}>
                                    MOST POPULAR
                                </div>
                            )}
                            <div style={{ 
                                ...
                                height: '100%',
                                boxShadow: isLight 
                                    ? (plan.highlighted ? '0 20px 40px rgba(99, 102, 241, 0.15)' : '0 10px 30px rgba(0,0,0,0.03)')
                                    : (plan.highlighted ? '0 20px 40px rgba(99, 102, 241, 0.2)' : '0 10px 30px rgba(0,0,0,0.4)')
                            }}>
                                ...
                                <div style={{ 
                                    ...
                                    <h3 style={{ fontSize: 22, fontWeight: 700, marginBottom: 12, fontFamily: 'var(--font-heading)' }}>{plan.name}</h3>
                                    <div style={{ fontSize: 32, fontWeight: 900, marginBottom: 16, color: plan.highlighted ? '#6366f1' : (isLight ? '#000' : 'var(--zy-white)') }}>{plan.price}</div>
                                    <p style={{ color: isLight ? '#666' : 'var(--zy-gray-text)', marginBottom: 32, fontSize: 15, lineHeight: 1.6 }}>{plan.desc}</p>
                                        ...
                                        {plan.features.map((f, j) => (
                                            <li key={j} style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
                                                <div style={{ 
                                                    ...
                                                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={plan.highlighted ? '#6366f1' : (isLight ? '#000' : 'var(--zy-white)')} strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
                                                </div>
                                                <span style={{ fontWeight: 600, fontSize: 15, color: isLight ? '#333' : '#ddd' }}>{f}</span>
                                            </li>
                                        ))}
                                    </ul>
                                    ...
                                    <button 
                                        ...
                                        style={Object.assign({ width: '100%', marginTop: 'auto', padding: '16px', fontSize: 16 }, 
                                            plan.highlighted ? { background: '#6366f1', borderColor: '#6366f1', color: '#fff', boxShadow: '0 10px 20px rgba(99, 102, 241, 0.3)' } 
                                            : (isLight ? { borderColor: '#ddd', color: '#000', background: 'transparent' } : { borderColor: 'rgba(255,255,255,0.2)', color: 'var(--zy-white)', background: 'transparent' })
                                        )}
                                    >
