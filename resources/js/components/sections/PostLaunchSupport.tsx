...
import { Bot, Brain, Clock, Search, DollarSign, Shield, Smartphone, Layers, WifiOff, Fingerprint, Bell, Link, Target, TrendingUp, PieChart, Globe, Split, Repeat, Activity, Cpu, Wifi, Battery, Share2, HardDrive, Box, Maximize, Users, GitCommit, Database, Zap, Monitor, Edit, Eye, Server } from 'lucide-react';

const IconMap: any = {
    'robot': Bot, 'brain': Brain, 'clock': Clock, 'search': Search, 'dollar-sign': DollarSign, 'shield': Shield,
    'smartphone': Smartphone, 'layers': Layers, 'wifi-off': WifiOff, 'fingerprint': Fingerprint, 'bell': Bell, 'link': Link,
    'target': Target, 'trending-up': TrendingUp, 'pie-chart': PieChart, 'globe': Globe, 'split': Split, 'repeat': Repeat,
    'activity': Activity, 'cpu': Cpu, 'wifi': Wifi, 'battery': Battery, 'share-2': Share2, 'hard-drive': HardDrive,
    'box': Box, 'maximize': Maximize, 'users': Users, 'git-commit': GitCommit, 'database': Database, 'zap': Zap, 'monitor': Monitor,
    'edit': Edit, 'eye': Eye, 'server': Server
};
    ...
    return (
        <section className="zy-section" style={{ background: isLight ? '#FFFFFF' : 'var(--zy-black)' }}>
            <div style={{ maxWidth: 1200, margin: '0 auto', textAlign: 'center' }}>
                <span className="zy-section-label" style={{ color: themeColor, opacity: 1 }}>Long-Term Partnership</span>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 32 }}>
                    {supportFeatures.map((feature, i) => {
                        ...
                        const colors = [themeColor, '#3b82f6', '#10b981', '#f59e0b', '#ec4899'];
                        const featureColor = colors[i % colors.length];

                        return (
                            <div key={i} className="support-card-wrapper" style={{ transition: 'transform 0.3s' }}>
                                <GradientCard themeColor={featureColor} style={{ height: '100%', background: isLight ? '#fdfdfd' : 'var(--zy-gray-card)', boxShadow: isLight ? '0 10px 30px rgba(0,0,0,0.03)' : '0 10px 30px rgba(0,0,0,0.3)' }}>
                                    <div style={{ padding: '40px 32px', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                                        <div style={{ 
                                            ...
                                            color: featureColor, marginBottom: 24,
