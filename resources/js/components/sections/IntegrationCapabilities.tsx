...
import { Network, Cloud, Database, CreditCard, MessageSquare, Shield, Box, Code } from 'lucide-react';
...
const getIcon = (type: string) => {
    switch(type.toLowerCase()) {
        ...
        case 'database':
        case 'vector db': return <Database size={24} />;
        case 'communication':
        case 'email': return <MessageSquare size={24} />;
        case 'security':
        case 'sso': return <Shield size={24} />;
        case 'framework':
        case 'llm':
        case 'models': return <Code size={24} />;
    ...
    return (
        <section className="zy-section" style={{ background: isLight ? '#FFFFFF' : 'var(--zy-black)', overflow: 'hidden' }}>
            <div style={{ maxWidth: 1200, margin: '0 auto', textAlign: 'center', marginBottom: 40 }}>
                <span className="zy-section-label" style={{ color: themeColor, opacity: 1 }}>Connected Ecosystem</span>
            ...
            <div className="marquee-container" style={{ position: 'relative', width: '100%', padding: '20px 0' }}>
                <div className="marquee-track" style={{ display: 'flex', gap: 24, width: 'max-content', animation: 'marqueeScroll 25s linear infinite' }}>
                    {marqueeItems.map((tool, i) => (
                        ...
                        } as any} className="integration-card">
