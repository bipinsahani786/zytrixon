    ...
    return (
        <section className="zy-section" style={{ background: isLight ? '#f9fafb' : 'var(--zy-black)' }}>
            <div style={{ maxWidth: 1200, margin: '0 auto', textAlign: 'center' }}>
                <span className="zy-section-label" style={{ color: themeColor, opacity: 1 }}>{methodology.title}</span>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 24 }}>
                    {steps.map((step, i) => (
                        <div key={i} className="methodology-card" style={{ transition: 'transform 0.3s' }}>
                            <GradientCard themeColor={themeColor} style={{ height: '100%', background: isLight ? '#fff' : 'var(--zy-gray-card)', boxShadow: isLight ? '0 10px 30px rgba(0,0,0,0.03)' : 'none' }}>
