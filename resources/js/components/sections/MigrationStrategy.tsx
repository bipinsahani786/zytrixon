...
export default function MigrationStrategy({ service }: any) {
    ...
    return (
        <section className="zy-section" style={{ background: isLight ? '#fdfdfd' : 'var(--zy-black)', overflow: 'hidden' }}>
            <div style={{ maxWidth: 1200, margin: '0 auto', textAlign: 'center' }}>
                <span className="zy-section-label" style={{ color: '#10b981', opacity: 1 }}>Zero Downtime</span>
                <h2 className="zy-section-title" style={{ color: isLight ? '#000' : 'var(--zy-white)' }}>Legacy Migration Strategy</h2>
                <p style={{ color: isLight ? '#555' : 'var(--zy-gray-text)', lineHeight: 1.8, fontSize: 18, maxWidth: 800, margin: '0 auto 60px' }}>
                    ...
                    <div className="migration-flow" style={{ display: 'flex', justifyContent: 'center', width: '100%', gap: 32, flexWrap: 'wrap' }}>
                        {steps.map((step, i) => (
                            <React.Fragment key={i}>
                                ...
                                {i < steps.length - 1 && (
                                    <div className="migration-arrow" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', color: isLight ? '#ccc' : 'rgba(255,255,255,0.1)' }}>
