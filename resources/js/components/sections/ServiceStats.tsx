...
export default function ServiceStats({ service, location }: any) {
    ...
    const stats = [
        ...config.stats,
        { value: '24/7', label: location ? `Support in ${location.name}` : 'Global Support' }
    ];

    return (
        <section className="zy-section" style={{ background: isLight ? '#f9fafb' : 'var(--zy-black)', borderBottom: `1px solid ${isLight ? '#e5e7eb' : 'rgba(255,255,255,0.05)'}` }}>
            <div style={{ maxWidth: 1200, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 24, textAlign: 'center' }}>
                {stats.map((stat, i) => (
                    ...
                    >
                        <h3 style={{ fontSize: 48, fontWeight: 900, color: themeColor, marginBottom: 12, fontFamily: 'var(--font-heading)' }}>{stat.value}</h3>
                        <p style={{ color: isLight ? '#666' : 'var(--zy-gray-text)', fontSize: 14, letterSpacing: '0.05em', textTransform: 'uppercase', fontWeight: 700, margin: 0 }}>{stat.label}</p>
                    </div>
                ))}
            </div>
        </section>
